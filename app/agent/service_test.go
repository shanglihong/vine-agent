...
func TestAgentAppService_ToolExecutionError(t *testing.T) {
	...
	assert.True(t, errors.As(recvErr, &notFoundErr))
	...
	for _, r := range results {
		...
		if r.Type == message.StreamMessageToolResult {
			...
			var notFoundErr *appagent.ToolNotFoundError
			assert.True(t, errors.As(r.ToolResult.Error, &notFoundErr))
		}
	}

	assert.True(t, hasToolCall, "should contain tool call message")
	assert.True(t, hasToolResult, "should contain tool result message with error")
	...
	assert.NotNil(t, sess.Messages[1].ToolCalls)
}
